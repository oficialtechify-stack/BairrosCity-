...
export function companyToPlace(company: Company): Place {
  ...
  return {
    ...
    rating: typeof company.rating === 'number' ? company.rating : (Array.isArray(company.reviews) && company.reviews.length > 0 ? Number((company.reviews.reduce((acc: number, r: any) => acc + (r.rating || 0), 0) / company.reviews.length).toFixed(1)) : 0),
    reviewsCount: typeof company.reviewsCount === 'number' ? company.reviewsCount : (Array.isArray(company.reviews) ? company.reviews.length : 0),
      ...
      (snapshot) => {
        ...
        snapshot.forEach((docSnap) => {
          ...
          list.push({
            ...
            isRegisteredCompany: true,
            rating: typeof data.rating === 'number' ? data.rating : (Array.isArray(data.reviews) && data.reviews.length > 0 ? Number((data.reviews.reduce((acc: number, r: any) => acc + (r.rating || 0), 0) / data.reviews.length).toFixed(1)) : 0),
            reviewsCount: typeof data.reviewsCount === 'number' ? data.reviewsCount : (Array.isArray(data.reviews) ? data.reviews.length : 0),
